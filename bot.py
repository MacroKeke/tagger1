...
async def info(event):
  await event.reply("**Merhaba Benim Ve Sahibim Hakkında Bilgi\n\nPython: 3.8.2\nKütüphanem: Telethon\n\nSahibim:  Gruplarınızda Üyeleri Etiketlemek için Yaratılmışım**",
                    buttons=(
                      [
                       Button.url('Beni Grubuna Ekle ➕', 'https://t.me/MajesteTaggerBot?startgroup=a')
                      ],
                      [
...
async def handler(event):
    async for usr in client.iter_participants(event.chat_id):
     ad = f"[{usr.first_name}](tg://user?id={usr.id}) "
     await event.edit(f"**Merhaba Ben @MajesteTaggerBot\nGrubunuzdakı Üyeleri Etiketleye Bilirim\nKomutlar için Komutlar Düğmesine Tıklaya Bilirsiz**", buttons=(
                      [
                       Button.inline("Komutlar", data="komutlar")
                      ],
                      [
...
async def handler(event):
    await event.edit(f"**Komutlarım:\n\n/all - Bütün üyeleri etiketlerim\n/atag - Sadece Yöneticileri etiketlerim\n/btag - Bayraklar ile etiket atarım\n/soztag - ömrüm vb sözler ile etiketlerim\etag - emojili etiketlerim\n/tag - 5 li etiket atarım\n/tektag - tekli etiket atarım\n/cancel - İşlemi Durdururum...\n\n❕ Yalnızca yöneticileri bu komutları kullanabilir.**", buttons=(
                      [
